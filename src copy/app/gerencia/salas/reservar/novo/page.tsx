...
export default function NovaReserva() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setConflito(false); // Ao alterar qualquer campo, libera o botão se estava bloqueado por conflito
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msgErro = (typeof error?.message === 'string') ? error.message.toLowerCase() : '';
      // Extrai apenas a mensagem após o último 'error:' se existir
      const msgLimpa = msgErro.split('error:').pop()?.trim() || msgErro;
      if (msgLimpa.includes("já existe") || msgLimpa.includes("conflito") || msgLimpa.includes("reserva")) {
        setMsg("Já existe uma reserva para essa data e horário!");
        setConflito(true);
      } else {
    ...
    <div className="form-container" style={{margin: '2.5rem auto', maxWidth: 420}}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          ...
          style={{background: conflito ? '#ccc' : '#ffa500', color: '#fff', marginTop: '0.5rem', cursor: conflito ? 'not-allowed' : 'pointer'}}
